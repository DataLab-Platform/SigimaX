...
"""
SigimaX Options System (:mod:`sigimax.options`)
------------------------------------------------
...
- **Simple API**: Options are accessed via ``options.color_mode.get()`` and
  ``options.color_mode.set("dark")``.
- **Context managers**: Temporarily override options with
  ``with options.fft_shift_enabled.context(False): ...``.
...
.. code-block:: python
    ...
    from sigimax.options import options

    # Get an option value
    colormap = options.ima_def_colormap.get()

    # Set an option value
    options.ima_def_colormap.set("gray")
    ...
    options.save()  # saves to default config path
    options.load()  # loads from default config path
...
class SigimaXOptions(AppOptionsContainer):
    ...
    def initialize_plotpy(self, config_app_name: str = "", load: bool = False) -> None:
        ...
        """
        import os.path as osp  # pylint: disable=import-outside-toplevel
